...
export class PizzasEffects {
    ...
    createPizzaSuccess$ = this.actions$.ofType(pizzaActions.CREATE_PIZZA_SUCCESS)
        .pipe(
            map((action: pizzaActions.CreatePizzaSucess) => action.payload),
        ...
        .pipe(
            ...
            switchMap(pizza => {
                return this.pizzaService.removePizza(pizza)
                    .pipe(
                        map(() => new pizzaActions.DeletePizzaSucess(pizza)),
    ...
    @Effect()
    handlePizzaSuccess$ = this.actions$.ofType(pizzaActions.DELETE_PIZZA_SUCCESS, pizzaActions.UPDATE_PIZZA_SUCCESS)
